...
function ScanContent() {
  ...
  const captureAndPredict = async () => {
    ...
    if (isUncertain || confidence < 60) {
      setResult({ disease: 'unknown', confidence });
      setTreatment({
        name: lang === 'si' ? 'හඳුනාගත නොහැකි රෝගයක්' : 'Disease Not Identified',
        symptoms: lang === 'si' ? 'පැහැදිලි රෝග ලක්ෂණ නොමැත' : 'No clear symptoms detected',
  ...
  // Image base64 localStorage හරහා pass කරනවා — URL හරහා නෙවෙයි
  if (capturedImage) {
    localStorage.setItem('govi_nena_last_scan_image', capturedImage);
  }
    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: '#F9FBF7' }}>
        <div style={{ background: 'linear-gradient(to bottom, #1B5E20, #4CAF50)', padding: 'clamp(40px, 6vw, 80px) clamp(20px, 5vw, 80px) clamp(24px, 4vw, 48px)', color: '#fff' }}>
          <h1 style={{ fontSize: 'clamp(24px, 3vw, 40px)', fontWeight: '700', margin: '0 0 8px' }}>
            {lang === 'si' ? 'බෝගය තෝරන්න' : 'Select Crop'}
          </h1>
          <p style={{ opacity: 0.8, fontSize: 'clamp(14px, 1.5vw, 20px)', margin: 0 }}>
            {lang === 'si' ? 'රෝගය හඳුනා ගැනීමට' : 'For disease detection'}
          </p>
        </div>
      ...
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: '#000' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: 'clamp(32px, 5vw, 60px) clamp(16px, 3vw, 40px) 16px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px' }}>
            ...
            <div style={{ background: 'rgba(253,216,53,0.2)', borderRadius: '12px', padding: '4px 14px' }}>
              <p style={{ color: '#FDD835', fontSize: 'clamp(11px, 1vw, 14px)', margin: 0, textAlign: 'center' }}>
                {lang === 'si'
                  ? `⚠️ ${CROPS[selectedCrop]?.si} කොළයක් විතරක් scan කරන්න`
                  : `⚠️ Only scan ${CROPS[selectedCrop]?.en} leaves`}
              </p>
            </div>
          </div>
        ...
        {modelLoading && (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '20px' }}>
            ...
            <p style={{ color: '#fff', fontSize: 'clamp(16px, 1.5vw, 24px)', margin: 0 }}>
              {lang === 'si' ? 'Model load වෙනවා...' : 'Loading model...'}
            </p>
            <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: 'clamp(12px, 1vw, 16px)', margin: 0 }}>
              {lang === 'si' ? 'මෙය මිනිත්තු 1ක් ගත වේ' : 'This may take a minute'}
            </p>
          </div>
        )}
        ...
        {!modelLoading && !modelError && (
          <>
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '20px', padding: '20px' }}>
              <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: 'clamp(13px, 1.2vw, 18px)', textAlign: 'center', margin: 0 }}>
              ...
              <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 'clamp(11px, 1vw, 15px)', textAlign: 'center', margin: 0 }}>
      ...
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: '#F9FBF7' }}>
        ...
        <div style={{ background: '#1B5E20', padding: 'clamp(36px, 5vw, 64px) clamp(20px, 4vw, 60px) clamp(20px, 3vw, 40px)', borderRadius: '0 0 24px 24px', boxShadow: '0 4px 20px rgba(0,0,0,0.15)', position: 'sticky', top: 0, zIndex: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px', maxWidth: '1200px', margin: '0 auto' }}>
            ...
            <div style={{ flex: 1 }}>
              <h1 style={{ color: '#fff', fontSize: 'clamp(18px, 2vw, 28px)', fontWeight: '600', margin: 0 }}>Analysis Results</h1>
              <p style={{ color: 'rgba(255,255,255,0.8)', fontSize: 'clamp(12px, 1vw, 16px)', margin: '2px 0 0' }}>විශ්ලේෂණ ප්‍රතිඵල</p>
            </div>
        ...
        <div style={{ flex: 1, padding: 'clamp(20px, 3vw, 48px) clamp(16px, 5vw, 80px)', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 'clamp(16px, 2vw, 28px)', alignContent: 'start', maxWidth: '1400px', margin: '0 auto', width: '100%', boxSizing: 'border-box', paddingBottom: '48px' }}>
          ...
          {capturedImage && (
            <div style={{ background: '#fff', borderRadius: 'clamp(16px, 2vw, 28px)', boxShadow: '0 4px 16px rgba(0,0,0,0.08)', padding: 'clamp(16px, 2vw, 28px)' }}>
              <h3 style={{ color: '#1B5E20', fontSize: 'clamp(16px, 1.5vw, 22px)', fontWeight: '600', marginBottom: '14px' }}>
                {lang === 'si' ? 'ග්‍රහණය කළ රූපය' : 'Captured Image'}
              </h3>
          ...
          <div style={{ background: '#fff', borderRadius: 'clamp(16px, 2vw, 28px)', boxShadow: '0 4px 16px rgba(0,0,0,0.08)', padding: 'clamp(20px, 2.5vw, 36px)' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
              <span style={{ color: '#1B5E20', fontSize: 'clamp(13px, 1.2vw, 18px)' }}>{lang === 'si' ? 'විශ්වාසය' : 'Confidence'}</span>
            ...
            <div style={{ height: 'clamp(8px, 1vw, 14px)', background: '#e0e0e0', borderRadius: '8px', overflow: 'hidden' }}>
              <div style={{
                ...
                background: result.confidence >= 80 ? '#4CAF50' : result.confidence >= 60 ? '#FDD835' : '#FF9800',
          ...
          <div style={{ background: '#fff', borderRadius: 'clamp(16px, 2vw, 28px)', boxShadow: '0 4px 16px rgba(0,0,0,0.08)', padding: 'clamp(20px, 2.5vw, 36px)' }}>
            <h3 style={{ color: '#1B5E20', fontSize: 'clamp(17px, 1.8vw, 26px)', fontWeight: '600', marginBottom: '14px' }}>
              {lang === 'si' ? 'රෝග ලක්ෂණ / Symptoms' : 'Symptoms / රෝග ලක්ෂණ'}
            </h3>
          ...
          <div style={{ background: '#fff', borderRadius: 'clamp(16px, 2vw, 28px)', boxShadow: '0 4px 16px rgba(0,0,0,0.08)', padding: 'clamp(20px, 2.5vw, 36px)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              {[
                { num: 1, label: lang === 'si' ? 'රසායනික ප්‍රතිකාර' : 'Chemical Treatment', text: treatment.chemical, color: '#1565C0' },
                { num: 2, label: lang === 'si' ? 'කාබනික ප්‍රතිකාර' : 'Organic Treatment', text: treatment.organic, color: '#2E7D32' },
                { num: 3, label: lang === 'si' ? 'වැළැක්වීම' : 'Prevention', text: treatment.prevention, color: '#E65100' },
              ].map((s) => (
          ...
          {/* View Full Details — result-detail page எக்கட் navigate */}
          <button onClick={handleViewDetails} style={{ background: '#fff', border: '2px solid #4CAF50', borderRadius: 'clamp(16px, 2vw, 28px)', padding: 'clamp(18px, 2vw, 28px) clamp(20px, 2.5vw, 32px)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 4px 16px rgba(0,0,0,0.06)', cursor: 'pointer', width: '100%' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
              ...
              <div style={{ textAlign: 'left' }}>
                <p style={{ color: '#1B5E20', fontSize: 'clamp(16px, 1.5vw, 22px)', fontWeight: '600', margin: 0 }}>View Full Details</p>
                <p style={{ color: '#795548', fontSize: 'clamp(12px, 1vw, 16px)', margin: '2px 0 0' }}>සම්පූර්ණ තොරතුරු බලන්න</p>
              </div>
            </div>
          ...
          <button onClick={() => router.push('/heatmap')} style={{ background: 'linear-gradient(to right, #1B5E20, #4CAF50)', borderRadius: 'clamp(16px, 2vw, 28px)', padding: 'clamp(18px, 2vw, 28px) clamp(20px, 2.5vw, 32px)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', border: 'none', cursor: 'pointer', width: '100%', boxShadow: '0 4px 20px rgba(46,125,50,0.3)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
              ...
              <div style={{ textAlign: 'left' }}>
                <p style={{ color: '#fff', fontSize: 'clamp(16px, 1.5vw, 22px)', fontWeight: '600', margin: 0 }}>Check Outbreak Map</p>
                <p style={{ color: 'rgba(255,255,255,0.8)', fontSize: 'clamp(12px, 1vw, 16px)', margin: '2px 0 0' }}>ව්‍යාප්ති සිතියම බලන්න</p>
              </div>
            </div>
            <svg width="28" height="28" viewBox="0 0 24 24" fill="none">
